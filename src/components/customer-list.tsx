  ...
  const filteredCustomers = useMemo(() => {
    return customers.filter(c => {
      if (filter.customerType.length > 0) {
        const multi = Array.isArray(c.customer_type_multi)
          ? (c.customer_type_multi as string[])
          : [];
        const types = [c.customer_type, ...multi].filter(Boolean) as string[];
        if (!filter.customerType.some(t => types.includes(t))) return false;
      }
      if (filter.address) {
        const q = filter.address.toLowerCase();
        const matched = [c.address, c.address_jibun, c.address_road]
          .some(a => a && a.toLowerCase().includes(q));
        if (!matched) return false;
      }
      const totalUnpaid = Number((c as any).total_unpaid ?? 0);
      if (filter.minUnpaid && totalUnpaid < Number(filter.minUnpaid)) return false;
      if (filter.maxUnpaid && totalUnpaid > Number(filter.maxUnpaid)) return false;
      if (filter.startDate && c.created_at && new Date(c.created_at) < new Date(filter.startDate)) {
        return false;
      }
      if (filter.endDate && c.created_at && new Date(c.created_at) > new Date(`${filter.endDate}T23:59:59`)) {
        return false;
      }
      return true;
    });

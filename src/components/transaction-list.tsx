...
export function TransactionList() {
  ...
  // 전체 집계 데이터 계산
  const totalCount = searchTerm.trim() ? filteredCustomerSummaries.length : summaries.length;
                ...
                <>
                  <TableRow className="bg-blue-50">
                    <TableCell colSpan={7} className="border-2 border-gray-300 px-4 py-3 text-center text-lg font-bold text-blue-800">
                      🔍 &quot;{searchTerm}&quot; 고객 검색 결과 ({filteredCustomerSummaries.length}명)
                    </TableCell>
                  </TableRow>
                  {filteredCustomerSummaries.map((summary) => {
                    ...
                    return (
                      <TableRow key={summary.customer_id} className="hover:bg-blue-50 cursor-pointer border-b border-gray-200 h-16">
                        <TableCell className="border-2 border-gray-300 px-4 py-4 whitespace-nowrap text-base text-blue-700 underline font-medium w-32 min-w-[120px] max-w-[160px] text-center overflow-hidden text-ellipsis" onClick={() => router.push(`/customers/${customer.id}/transactions`)}>
                          {customer.name}
                        </TableCell>
